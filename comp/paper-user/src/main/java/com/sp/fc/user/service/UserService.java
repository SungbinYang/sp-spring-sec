...
public class UserService {
    ...
    public Map<Long, User> getUsers(List<Long> userIds) {
        return userRepository.findAllById(userIds).stream().collect(Collectors.toMap(User::getUserId, Function.identity()));
    }
    ...
    public void updateUserSchoolTeacher(Long userId, Long schoolId, Long teacherId) {
        userRepository.findById(userId).ifPresent(user -> {
            ...
            if (!user.getSchool().getSchoolId().equals(user.getTeacher().getSchool().getSchoolId())) {
                throw new IllegalArgumentException("해당 학교 선생님이 아닙니다.");
            }
            save(user);
        });
    }
    ...
    public Page<User> listStudent(Integer pageNum, Integer size) {
